...
//        cr = AppContext.getInstance().getContentResolver();
     ...
     */
    public Bitmap getBitmap(final String url, final String path, int width, int height, final ILoadImageCallback callback, Object... objects){
        Bitmap bitmap = getBitmapFromCache(path, width, height);
        //如果能够从缓存中获取符合要求的图片，则直接回调
        if (bitmap != null) {
            Log.e("zhao", "get bitmap from cache");
            callback.onLoadImageCallBack(bitmap, path, objects);
        } else {
            decodeBitmapFromPath(url, path, width, height, callback, objects);
        }
        return bitmap;
    }
...
//    ExecutorService tpe = Executors.newFixedThreadPool(1);
     ...
     * 通过path获取图片bitmap
     */
    private void decodeBitmapFromPath(final String url, final String path, final int width, final int height, final ILoadImageCallback callback, final Object... objects) throws OutOfMemoryError {
        ...
        tpe.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (width == 0 || height == 0) {
                    ...
                    options.inSampleSize = computeScale(options, ((WindowManager) (MyApp.getMyApp().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth(), ((WindowManager) (MyApp.getMyApp().getSystemService(Context.WINDOW_SERVICE))).getDefaultDisplay().getWidth());
                    ...
                    String hash = MD5.md5(path+"_"+width+"_"+height);
                    File file = new File(MyUtils.getDataPath());
                    if (!file.exists())
                        file.mkdirs();
                    //临时文件的文件名
                    String tempPath = MyUtils.getDataPath() + hash + ".temp";
                    File picFile = new File(path);
                    if (!picFile.exists()){
                        String localPath = Environment
                                .getExternalStorageDirectory().getAbsolutePath()+MyUtils.getFileName(url);
